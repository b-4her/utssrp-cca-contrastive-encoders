# Contrastive Residual Encoder Experiments

This folder contains a small experiment for comparing linear encoders with
residual MLP nonlinear encoders under three synthetic paired-data settings.
...
- `encoder-comparison.ipynb` is the main notebook. Run this from top to bottom.
- `contrastive_encoders/` contains the reusable Python code imported by the notebook.
- [report-plots/](report-plots/README.md) contains saved PNG figures from the
  notebook.
- [contrastive-encoder-comparison-final-slides.pptx](contrastive-encoder-comparison-final-slides.pptx)
  is the slide deck for this experiment.
...
`report-plots/`. File names are generated from the plot titles so they are easy
to match with the notebook sections.
...
- `data.py`
  Generates synthetic paired datasets, including independent noise, linear
  latent relationships, and cubic latent relationships.

- `losses.py`
  Implements the paper contrastive objective exactly as a PyTorch loss.

- `regularization.py`
  Contains L1, L2, and nonlinear-output penalties.

- `metrics.py`
  Computes alignment, retrieval, correlation, and latent recovery metrics.

- `training.py`
  Trains one model on one dataset and returns all diagnostics.

- `experiments.py`
  Builds the model grid, creates all datasets, and runs the full comparison.

- `plotting.py`
  Renames result columns for display and creates the notebook plots.
...
1. Open `encoder-comparison.ipynb`.
...
6. Run the sweep sections when you want the slower alpha and signal/noise
   comparisons.
7. Use the generated PNG files in `report-plots/` for your report.
